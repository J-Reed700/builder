...
pub enum RemoteCommand {
    /// Connect this workspace to a Builder Gateway over an outbound WebSocket.
    Connect {
        /// Exact public gateway origin, e.g. https://builder.example.com.
        gateway: String,
        /// Single-use code generated by the authenticated gateway dashboard.
        #[arg(long)]
        code: Option<String>,
        /// Pair successfully and exit; intended for background-service installers.
        #[arg(long)]
        pair_only: bool,
    },
}
...
mod tests {
    ...
    #[test]
    fn remote_keeps_direct_mode_and_adds_gateway_pairing() {
        let direct = Cli::try_parse_from(["builder", "remote"]).unwrap();
        assert!(matches!(
            direct.command,
            Some(Command::Remote { command: None, .. })
        ));
        let connected = Cli::try_parse_from([
            "builder",
            "-C",
            "project",
            "remote",
        ...
        assert!(matches!(
            connected.command,
            Some(Command::Remote {
                command: Some(RemoteCommand::Connect { .. }),
                ..
            })
        ));
    }
