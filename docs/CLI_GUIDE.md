...
While summarizing, the terminal shows a progress bar estimated from how much of the older transcript has been summarized and how much of the current fragment's handoff has been written. llama.cpp can also report prompt processing, which dominates summarization time on local models; add `return_progress = true` under the profile's `[profiles.NAME.extra_body]` to see "reading N of M prompt tokens" in the bar and in ordinary request status. Leave it unset for servers that reject unknown request fields. The browser shows the same estimate as a percentage.

While the model writes an action, such as a large edit after a line of prose, the status line counts the characters streamed so far (`Preparing Edit file · 7.2k chars written`). A long edit on a local model is still generating even when no new text appears.
...
- **Crash recovery.** The authoritative conversation journal uses SQLite WAL, `synchronous=FULL`, `IMMEDIATE` multi-statement writes, and a versioned schema. Current-schema opens do not request its writer lock. Rebuildable code indexes use a failure-isolated WAL sidecar and a checkout-scoped maintenance lock, so bulk or duplicate background indexing cannot lock conversation persistence. An exclusive session lock prevents two processes from driving the same session.
...
The agent can `list_files`, `read_file`, `search`, `code_search`, `write_file`, `edit_file`, `multi_edit`, `todo_write`, `subagent`, and `shell`.
...
Consecutive reads, searches, file listings and subagent calls in one model response run in parallel (up to `parallel_tools`, default 8). They are still claimed, recorded and displayed in call order, and a group never spans an edit or command, so a read after an edit sees the edit. A side-effect-free call interrupted before its result was saved closes as a retryable failure instead of an uncertain outcome that pauses the session.

`subagent` delegates a read-only investigation to a separate agent with its own context. A subagent sees only `list_files`, `read_file`, `search` and `code_search`, runs under read-only approval whatever the parent's mode, and cannot start further subagents. Only its final report returns to the main conversation, so a broad investigation does not fill the main context with file contents. While one runs, the status line shows its latest action; its row shows how many tool calls it made. Several subagents run at once, up to `subagent_parallel` (default 3; match your server's parallel request slots, for example llama.cpp `--parallel`). Each has `subagent_rounds` model rounds (default 30); a subagent that reaches its limit is asked once, without tools, to report what it found. Its transcript is saved as a linked child session: it is excluded from `builder sessions` and the browser chat list, and the report's session prefix works with `builder export`. Disable with `config pipeline set subagents=false`.
