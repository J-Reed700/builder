...
| `builder doctor` | Validate config, open storage, and check model discovery |
...
Resume uses the saved workspace and profile name. Profiles are resolved at startup, so editing a profile's endpoint/model changes subsequent runs. `--profile` can explicitly override the saved profile. No API keys are copied to the session. `doctor` checks discovery, not generation quality or tool support.
