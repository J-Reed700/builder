...
- The transcript keeps what the model wrote beside each tool call, so a turn reads as explanation, then a folded list of calls, then the tool results. Models that stream reasoning (`reasoning_content`, such as Qwen3 on llama.cpp) get a folded **Thinking** section per turn and a live one while the run is in progress. Reasoning is stored with the chat for reading only; it is never sent back to the model, never summarized during compaction, and never printed by the terminal.
